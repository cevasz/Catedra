...
/// La compilación de trabajo (`--flavor dev`): «Cátedra Dev».
...
final updateCheckProvider = FutureProvider<UpdateCheck>((ref) async {
  ...
  // «Cátedra Dev» se instala por cable desde el equipo: las versiones de
...
class UpdateDownloader extends StateNotifier<UpdateDownload> {
  ...
  Future<void> start(UpdateManifest update) async {
    ...
    final file = File(p.join(dir.path, 'updates', 'catedra-${update.versionCode}.apk'));
    var lastPercent = -1;
