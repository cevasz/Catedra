...
import 'package:catedra/core/platform/update_channel.dart';
import 'package:catedra/domain/updates/update_manifest.dart';
import 'package:catedra/features/updates/application/update_providers.dart';
...
void main() {
  final update = UpdateManifest(
    versionCode: 7,
    versionName: '0.2.0',
    apk: Uri.parse('https://example.com/catedra.apk'),
  );
  late Directory tmp;

  setUp(() => tmp = Directory.systemTemp.createTempSync('catedra_updates'));
  ...
  test('descarga a cache/updates y abre el instalador', () async {
    ...
    expect(channel.opened?.path, endsWith('/updates/catedra-7.apk'));
