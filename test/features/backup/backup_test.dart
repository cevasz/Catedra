...
import 'package:catedra/features/backup/data/backup.dart';
...
void main() {
  ...
  Future<(Uint8List, int)> exported() async {
    final db = CatedraDatabase.forTesting(NativeDatabase(File('${tmp.path}/app.sqlite')));
  ...
  test('una copia exportada se valida y trae sus datos', () async {
    ...
    final db = CatedraDatabase.forTesting(NativeDatabase(target));
  ...
  test('otra base SQLite no es de Cátedra', () {
  ...
  test('el nombre del archivo lleva la fecha', () {
    expect(Backup.fileName(DateTime(2026, 9, 4)), 'catedra-2026-09-04.catedra');
    expect(Backup.fileName(DateTime(2026, 9, 4), prefix: 'catedra-dev'), 'catedra-dev-2026-09-04.catedra');
  });
}
