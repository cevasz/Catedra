#!/usr/bin/env bash
# Publica una versión de Cátedra por GitHub Releases (§43).
#
...
NOTES="${2:-Cátedra $VERSION}"
REPO="cevasz/Catedra"
KEYSTORE="${CATEDRA_KEYSTORE:-$HOME/.android/debug.keystore}"
...
if ! gh secret list -R "$REPO" | grep -q ANDROID_KEYSTORE_BASE64; then
  ...
  gh secret set ANDROID_KEYSTORE_PASSWORD -R "$REPO" -b "${CATEDRA_STORE_PASSWORD:-android}"
  gh secret set ANDROID_KEY_ALIAS -R "$REPO" -b "${CATEDRA_KEY_ALIAS:-androiddebugkey}"
  gh secret set ANDROID_KEY_PASSWORD -R "$REPO" -b "${CATEDRA_KEY_PASSWORD:-android}"
fi
