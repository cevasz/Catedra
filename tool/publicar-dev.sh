#!/usr/bin/env bash
# Entrega una versión de «Cátedra Dev» (§52): la compila en este equipo (va
...
if ! gh release view dev -R "$REPO" >/dev/null 2>&1; then
  gh release create dev -R "$REPO" --prerelease --target "$(git rev-parse HEAD)" \
    --title "Cátedra Dev (versión de desarrollo)" \
...
echo "Cátedra Dev $NAME (versionCode $CODE) publicada. La Dev la ofrece al abrirse."
